...
(function(document, w, requestAnimFrame) {
   ...
   MapPoint.prototype.near = function(ctx) {
      return ctx.y <= this.y + 2 && this.y <= ctx.y && 
             ctx.x <= this.x + 2 && this.x <= ctx.x && 
             ctx.under == this.under;
   }
   ...
   // Update game objects
   CanvasContext.prototype.update = function (delta) {
   ...
   CanvasContext.prototype.render = function () {
      ...
      ctx.save()
      ctx.beginPath()
      ctx.arc(player.x, player.y, s.PLAYER_RADIUS + 3, d, e, !1)
      ctx.closePath();
      var f = ctx.createRadialGradient(player.x, player.y, 1, player.x, player.y, s.PLAYER_RADIUS + 6);
      ...
      var angle = -(player.viewAngle * Math.PI / 180);
      ctx.beginPath()
      ...
      g.addColorStop(2 / 3, "rgba(" + s.FOV_COLOR + ", 0.33)")
      ...
      // Paint circle according to player team
      ctx.beginPath()
      ctx.moveTo(player.x, player.y)
      ctx.arc(player.x, player.y, s.PLAYER_RADIUS, d, e, !1)
      ...
      // Draw player name
      if (player.team == s.COUNTER_TERRORIST)
   ...
   CanvasContext.prototype.animate = function (then) {
      ...
      requestAnimFrame(self.animate.bind(self, now));
   };
